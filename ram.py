...
#The function cusp_ramification actually computes the ramification
...
def makeData_from_label(file_name, search_label):
	for line in open(file_name + ".txt", "r"):
		label = line.split(":")[0]
		if label == search_label:
			...
			nu3 = ZZ(line.split(":")[6])
			genus = ZZ(line.split(":")[7])
			return makeData([label, nn, level, index, Gammabar_gens, nu2, nu3, genus])
...
def profile_orbits(genList, ell, n):
	...
	OrbitIndices = [i for i in xrange(len(GammaN_orbits))]

	#we'll keep track of cusp reps by their position in the list GammaN_orbits

	for h in genLifts:
		#print "h is", h
		for oi in xrange(len(OrbitIndices)):
	...
	for i in xrange(len(OrbitIndices)):
		r = OrbitIndices[i]
		if r != i:
			s = i
			value_same = False
...
def cusp_ramification(genList, ell, n):
	...
	for c in Set(cusps):
		...
		for i in cusps:
			if i == c:
				ctr += 1
		ram_data.append([c,(N*ctr)/ambient_size(genList, ell, n)])
...
def fiber_genus(Data1, Data2):
	...
	g += ee
	#print "The contribution from cusps is", ee
	return g
